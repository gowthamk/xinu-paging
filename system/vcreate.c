...
#define	vroundew(x)	( (x+3)& ~0x3)

/*------------------------------------------------------------------------
 *  vcreate  -  Create a process to start running a function on x86 with paging.
 *------------------------------------------------------------------------
 */
pid32	vcreate(
	...
	)
{
	...
	if (ssize < MINSTK)
		ssize = MINSTK;
	ssize = (uint32) vroundew(ssize);
	if (((saddr = (uint32 *)getstk(ssize)) ==
	    (uint32 *)SYSERR ) ||
	    (pid=vnewpid()) == SYSERR || priority < 1 ) {
		restore(mask);
		return SYSERR;
	}
...
/*------------------------------------------------------------------------
 *  vnewpid  -  Obtain a new (free) process ID
 *------------------------------------------------------------------------
 */
local	pid32	vnewpid(void)
{
